...
def train(img_size, device=torch.device('cpu'), learning_rate=1e-3, num_epochs=500, decay_step=5, gamma=0.98,
          num_classes=2, lambda_=0.5, m_plus=0.9, m_minus=0.1, checkpoint_folder=None, checkpoint_name=None, load_checkpoint=False, graphs_folder=None):
    '''
    Function to train the DeepCaps Model
    '''
    checkpoint_path = checkpoint_folder + checkpoint_name

    deepcaps = DeepCapsModel(num_class=num_classes, img_height=img_size,
                             img_width=img_size, device=device).to(device)  # initialize model
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer=optimizer, step_size=decay_step, gamma=gamma)
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx, (train_data, labels) in tqdm(enumerate(train_loader)):  # from training dataset
            ...
            onehot_label = onehot_encode(
                labels, num_classes=num_classes, device=device)
            # print(onehot_label)

            optimizer.zero_grad()
            outputs, _, reconstructed, indices = deepcaps(data, onehot_label)
            loss = deepcaps.loss(x=outputs, reconstructed=reconstructed, data=data,
                                 labels=onehot_label, lambda_=lambda_, m_plus=m_plus, m_minus=m_minus)
