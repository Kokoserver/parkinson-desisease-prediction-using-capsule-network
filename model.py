...
class Conv2DCaps(nn.Module):
    ...
    def forward(self, inputs):
        ...
        caps_reshaped = inputs.view(
            batch_size, self.conv_channel_in*self.caps_num_in, self.height, self.width)

        conv_output = self.conv(caps_reshaped)  # conv process.

        # the size of the feature map after applying convolution.
        height, width = conv_output.size()[-2:]

        # reshape the conv output to capsules.
        conv_reshaped = conv_output.view(
            batch_size, self.conv_channel_out, self.caps_num_out, height, width)

        # apply the activation function before returning the capsules.
        return squash(conv_reshaped, dim=2)
...
class Conv3DCaps(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        batch_size = x.size()[0]
        x = x.view(batch_size, self.conv_channel_in *
                   self.caps_num_in, self.height, self.width)
        x = x.unsqueeze(1)
        x = self.conv_3d(x)
        ...
        x = x.view(batch_size, self.conv_channel_in, self.conv_channel_out,
                   self.caps_num_out, self.height, self.width)
    ...
    def routing(self, x, x_detached, batch_size, routing_iter=3):
        ...
        for iter_idx in range(routing_iter):
            ...
            temp = self.B.permute(0, 5, 3, 1, 2, 4).contiguous().view(
                batch_size, self.conv_channel_in, 1, self.height*self.width*self.conv_channel_out)

            # apply softmax on the last dimension.
            k = func.softmax(temp, dim=-1)

            # After the softmax, we can reshape and permute the tensor to the way it was.
            k = k.view(batch_size, self.conv_channel_in, 1, self.width, self.height,
                       self.conv_channel_out).permute(0, 3, 4, 2, 5, 1).contiguous()
...
class FC_Caps(nn.Module):
    ...
    def __init__(self, output_capsules, input_capsules, in_dimensions, out_dimensions, device, routing_iter=3):
        ...
        self.W = nn.Parameter(torch.randn(
            1, self.input_capsules, self.output_capsules, self.out_dimensions, self.in_dimensions)*0.05)
        self.b = nn.Parameter(torch.randn(
            1, 1, self.output_capsules, self.out_dimensions)*0.05)
    ...
    def forward(self, x):
        ...
        b_ij = x.new_zeros(size=(x.size()[
                           0], self.input_capsules, self.output_capsules, 1), requires_grad=False).to(self.device)
        ...
        for iter_idx in range(self.routing_iter):
            ...
            if iter_idx == self.routing_iter - 1:
                # multiply with the original u_hat since we want the gradient flow.
                s_j = (c_ij * u_hat).sum(dim=1, keepdim=True) + self.b
                ...
                v_j = squash(s_j, dim=-1)
                a_ij = (u_hat_detached * v_j).sum(dim=-1,
                                                  keepdim=True)  # agreement check
...
class Mask_CID(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        # a tensor containing integer from 0 to batch size.
        batch_ind = torch.arange(start=0, end=batch_size).to(self.device)
        # records the label's index for every batch.
        m = torch.stack([batch_ind, max_len_indices], dim=1).to(self.device)
...
class Decoder(nn.Module):
    ...
    def __init__(self, caps_dimension, device, num_caps=1, img_size=28, img_channels=1):
        ...
        self.fc = nn.Sequential(torch.nn.Linear(self.caps_dimension*self.num_caps,
                                self.neurons*self.neurons*16), nn.ReLU(inplace=True)).to(self.device)
        ...
        self.reconst_layers2 = nn.ConvTranspose2d(
            in_channels=64, out_channels=32, kernel_size=3, stride=2, padding=1)
        self.reconst_layers3 = nn.ConvTranspose2d(
...
class DeepCapsModel(nn.Module):
    ...
    def __init__(self, num_class, img_height, img_width, device):
        ...
        self.bn1 = torch.nn.BatchNorm2d(
            num_features=128, eps=1e-08, momentum=0.99)
        ...
        self.conv2dcaps_30 = Conv2DCaps(height=height, width=width, conv_channel_in=32, caps_num_in=8, conv_channel_out=32,
                                        caps_num_out=8, stride=2, device=device)
        height, width = math.ceil(height/2), math.ceil(width/2)
        self.conv3dcaps_31 = Conv3DCaps(height=height, width=width, conv_channel_in=32,
                                        caps_num_in=8, conv_channel_out=32, caps_num_out=8, device=device)
        ...
        self.decoder = Decoder(caps_dimension=16, num_caps=1,
                               device=device, img_size=self.width, img_channels=1)
    ...
    def margin_loss(self, x, labels, lambda_, m_plus, m_minus):
        ...
        classification_loss = (labels*max_l + lambda_ *
                               (1-labels)*max_r).sum(dim=1)
    ...
    def reconstruction_loss(self, reconstructed, data):
        ...
        loss = self.mse_loss(reconstructed.view(
            batch_size, -1), data.view(batch_size, -1))
    ...
    def loss(self, x, reconstructed, data, labels, lambda_=0.5, m_plus=0.9, m_minus=0.1):
        ...
        '''
        total_loss = self.margin_loss(
            x, labels, lambda_, m_plus, m_minus) + self.reconstruction_loss(reconstructed, data)
